...
Genes<-c("Ccl4","Fos","Cd63","Spp1","Cx3cr1")
toptable[!(toptable$lab %in% Genes),]$lab<-NA
toptable$xvals <- toptable[['avg_log2FC']]
toptable$yvals <- -log10(toptable[['p_val_adj']])
xlab<-bquote(~Log[2] ~ "fold change")
ylab<-bquote(~-Log[10] ~ italic(P))
Volcano24<-ggplot(toptable, aes(x = xvals, y = yvals)) + geom_point(na.rm = TRUE,size=0.1,color=unname(ColorsTreatment["1d BBBO"]))+ labs(title = '1d BBBO v Sham')+xlab(xlab) + ylab(ylab) + geom_label_repel(aes(label=lab),size=3,label.padding = unit(0.1, "lines"))+theme(
...
Genes<-c("Spp1","Ccl12","Cd63","Ifi30")
toptable[!(toptable$lab %in% Genes),]$lab<-NA
toptable$xvals <- toptable[['avg_log2FC']]
toptable$yvals <- -log10(toptable[['p_val_adj']])
xlab<-bquote(~Log[2] ~ "fold change")
ylab<-bquote(~-Log[10] ~ italic(P))
Volcano72<-ggplot(toptable, aes(x = xvals, y = yvals)) + geom_point(na.rm = TRUE,size=0.1,fill=unname(ColorsTreatment["3d BBBO"]))+ labs(title = '3d BBBO v Sham')+xlab(xlab) + ylab(ylab) + geom_label_repel(aes(label=lab),size=3,label.padding = unit(0.1, "lines"))+theme(
  panel.grid = element_blank(),
  plot.title = element_text(size=10),
  plot.margin=margin(0.5,0.5,0.5,0.5,"cm"),
  panel.background = element_blank(),
...
Left<-plot_grid(Inset,BC2,Feat,nrow=3,labels=c("A","C","D"),rel_heights = c(1,1,1))
Phase<-plot_grid(HMPhase,BC1,nrow=1,align = "v",axis = "tb",rel_widths = c(1,1.25),labels = c("G","H"))
Right<-plot_grid(OntoPlot,Phase,Blank,nrow=3,rel_heights = c(2.5,1,2),labels = c("F","","I"))

Full<-plot_grid(Left,HM_Ready,Right,nrow=1,rel_widths = c(1,1.5,1.5),labels=c("","E",""))
ggsave("Proposal-MicrogliaDEG.png",width = 4,height = 6,units = "in",bg="white")
...
# Powerpoint
```{r}
Dim<-DimPlot(Microglia,group.by = "seurat_clusters",label=TRUE,cols = MicrogliaClusterColors,label.box = FALSE,label.size = 2)+scale_y_continuous(limits=c(-9.5,7))+scale_x_continuous(limits=c(-4.5,7.25))+theme(plot.title = element_blank(),
        axis.ticks = element_blank(),
        axis.text = element_blank(),
        axis.title = element_text(size=6),
        legend.position = "none",
        plot.margin = margin(0,1,0,0.3,"cm"))


Full<-plot_grid(Dim,BC2+theme(legend.key.height = unit(0.25,"cm"),axis.title = element_text(size=6),legend.key.width = unit(0.1,"cm"),axis.text = element_text(size=5),legend.title = element_text(size=6,angle=-90,hjust=0.5,vjust=0),legend.box.margin = margin(0,0,0,-0.5,"cm")),nrow=2,labels=c("A","B"),label_size = 8)
ggsave("Proposal-PPT-MicrogliaDEG_Clusters.png",width = 600,height = 800,units = "px",bg="white")
...
ggsave("Proposal-PPT-MicrogliaDEG.png",width = 4,height = 5,units = "in",bg="white")

Full<-plot_grid(BC1,Blank,nrow=1,labels=c("A","B"),label_size = 8)
ggsave("Proposal-PPT-MicrogliaDEG_CellCycle.png",width = 1000,height = 500,units = "px",bg="white")
```
