...
ggsave("IUS_Poster_UMAP.png",width=4,height=4,bg="white")
```
...
tab<-tab[tab$Var1!="Sham",]
tab$Var1<-recode(tab$Var1,"1d BBBO"="1d",
                 "3d BBBO"="3d")
BC2<-ggplot(tab,aes(x="",y=Freq_Norm,fill=Var2))+geom_bar(stat="identity", width=1) + coord_polar("y", start=0)+theme_void()+facet_wrap(.~Var1,nrow=1,strip.position = "bottom")+theme(legend.position = "none",strip.text = element_text(size=12))+scale_fill_manual(values=MicrogliaClusterColors2)
```
...
```{r}
plot_grid(Dim1,DAMVolc,Percent,nrow=1,labels=c("a","b","c"),rel_widths = c(1,1,1),align = T,axis = "b")
ggsave("IUS_Poster_DAM.png",width = 6,height = 1.5,units = "in",bg="white")
...
BC2
ggsave("IUS_Lecture_MDGE.png",width = 6,height = 2,units = "in",bg="white")
```
